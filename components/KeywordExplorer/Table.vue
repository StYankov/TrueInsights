            ...
            <tbody>
                <tr v-for="(row, i) in filteredData" :key="'row-' + i">
                    <td v-for="(value, key) in row" :key="key + i">
                        {{ Object.keys(value)[0] }}
                        <span class="position-up" v-if="getPositionChange(value) > 0">
                            <fa :icon="['fas', 'chevron-up']" />
                            {{ getPositionChange(value) }}
                        </span>
                        <span class="position-down" v-if="getPositionChange(value) < 0">
                            <fa :icon="['fas', 'chevron-down']" />
                            {{ Math.abs(getPositionChange(value)) }}
                        </span>
                    </td>
                </tr>
            </tbody>
...
export default {
    ...
    methods: {
        ...
        getPositionChange(rowData) {
            return rowData[Object.keys(rowData)[0]].position;
        }
    },
    computed: {
        filteredData() {
